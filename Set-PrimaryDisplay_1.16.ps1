...
try {
    ...
    # QDC_ONLY_ACTIVE_PATHS=2 -- at login only active paths are relevant
    $r=[SDC]::GetDisplayConfigBufferSizes(2,[ref]$np,[ref]$nm)
    L "BufferSizes ret=$r np=$np nm=$nm"
    if($r -ne 0 -or $np -eq 0){ L "No paths -- exiting"; exit 1 }
    # Pad buffers: at login timing can cause size mismatch
    $npBuf=[uint32]($np*2+4); $nmBuf=[uint32]($nm*2+4)
    $paths=New-Object SDC+PATH[] $npBuf; $modes=New-Object SDC+MODE[] $nmBuf
    $r=[SDC]::QueryDisplayConfig(2,[ref]$np,$paths,[ref]$nm,$modes,[IntPtr]::Zero)
    L "QueryDisplayConfig ret=$r np=$np nm=$nm"
    if($r -ne 0){ L "QDC failed ret=$r"; exit 1 }
