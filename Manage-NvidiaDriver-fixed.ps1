...
if (-not $isResume) {
    ...
    $s3GamingStr = if($script:S3CacheGaming.Error){'ERROR: '+$script:S3CacheGaming.ErrorMsg}else{$script:S3CacheGaming.Version}
    $s3GridStr   = if($script:S3CacheGrid.Error){'ERROR: '+$script:S3CacheGrid.ErrorMsg}else{$script:S3CacheGrid.Version}
    Write-Log ('[TIMING] S3 Gaming=' + $s3GamingStr + ' Grid=' + $s3GridStr) -Level 'INFO'
