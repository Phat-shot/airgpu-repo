...
function Set-PrimaryDisplayViaSDC {
    ...
    # Query with QDC_ALL_PATHS so inactive/virtual displays are included
    try {
        ...
        Write-Log "SDC: query exception: $_" -Level "WARN"
        return $false
    }

    # Scan all paths, log them, find target
    ...
    for ($p = 0; $p -lt $numPaths; $p++) {
        ...
        Write-Host "  [SDC]   path[$p] fname='$fname' avail=$available srcLUID=$srcLUID srcModeIdx=$srcModeIdx" -ForegroundColor DarkGray
        Write-Log "SDC: path[$p] fname='$fname' avail=$available srcLUID=$srcLUID srcModeIdx=$srcModeIdx"
        ...
        if (($null -eq $targetPathIdx) -and ($luidMatch -or $nameMatch -or $nvidiaMatch)) {
            $targetPathIdx = $p
    ...
    for ($m = 0; $m -lt $numModes; $m++) {
        ...
        Write-Host "  [SDC]   mode[$m] type=$it adapterLow=$adaptLow ${w}x${h} at ($x,$y)" -ForegroundColor DarkGray
        Write-Log "SDC: mode[$m] type=$it adapterLow=$adaptLow ${w}x${h} at ($x,$y)"
    }

    # Find source mode index for matched path
    $targetSrcIdx = $null
    if ($null -ne $targetPathIdx) {
        ...
        if ($si -ne 0xFFFFFFFF -and [int]$si -lt $modes.Count -and $modes[[int]$si].infoType -eq 2) {
            $targetSrcIdx = [int]$si
            Write-Log "SDC: srcIdx from modeInfoIdx=$targetSrcIdx"
        }
        if ($null -eq $targetSrcIdx) {
            ...
            for ($m = 0; $m -lt $modes.Count; $m++) {
                if ($modes[$m].infoType -eq 2 -and
                    ...
                    $targetSrcIdx = $m
                    Write-Log "SDC: srcIdx from adapter LUID match: $m"
                    break
                }
            }
        }
    ...
    # Fallback: largest valid source mode
    if ($null -eq $targetSrcIdx) {
    ...
    # Shift all source positions so target lands at (0,0) = primary
    ...
    Write-Log "SDC: shift srcIdx=$targetSrcIdx by (-$ox,-$oy)"
