...
function Set-PrimaryDisplayViaRegistry {
    ...
    $nvidiaConfigKey = $null
    $nvidiaDisplayKey = $null

    foreach ($cfg in $configs) {
        # Each config key has numbered subkeys (00, 01...) per display
        $dispKeys = Get-ChildItem $cfg.PSPath -ErrorAction SilentlyContinue
        foreach ($dk in $dispKeys) {
            $props = Get-ItemProperty $dk.PSPath -ErrorAction SilentlyContinue
            # Look for NVIDIA in the description
            $desc = $props.MonitorDescription
            if (-not $desc) { $desc = $props.Name }
            if (-not $desc) {
                # Try the Connectivity sibling key
                $connBase = "HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers\Connectivity"
                $connKey  = $connBase + "\" + $cfg.PSChildName
                $connProps = Get-ItemProperty $connKey -ErrorAction SilentlyContinue
                $desc = $connProps.MonitorDescription
            }
            Write-Host "  [REG]   $($cfg.PSChildName)\$($dk.PSChildName) desc='$desc'" -ForegroundColor DarkGray
            Write-Log "Registry: $($cfg.PSChildName)\$($dk.PSChildName) desc='$desc'"
            if ($desc -and $desc -like "*NVIDIA*") {
                $nvidiaConfigKey  = $cfg.PSPath
                $nvidiaDisplayKey = $dk.PSPath
            }
        }
    }

    if (-not $nvidiaDisplayKey) {
        Write-Host "  [REG] NVIDIA display config not found in CCD registry." -ForegroundColor Red
        Write-Log "Registry: NVIDIA config key not found" -Level "WARN"
        return $false
    }

    Write-Host "  [REG] Found NVIDIA config: $nvidiaDisplayKey" -ForegroundColor Cyan
    Write-Log "Registry: NVIDIA config at $nvidiaDisplayKey"

    # Set PrimSurf=1 on NVIDIA display, 0 on all others in same config
    $dispKeys2 = Get-ChildItem $nvidiaConfigKey -ErrorAction SilentlyContinue
    $errors = 0
    foreach ($dk in $dispKeys2) {
        $isPrimary = ($dk.PSPath -eq $nvidiaDisplayKey)
        try {
...
function Set-PrimaryDisplay {
    ...
    # Try registry/CCD approach -- session-independent, works over RDP
    $ok = Set-PrimaryDisplayViaRegistry -Target $Target
