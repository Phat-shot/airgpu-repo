...
function Set-PrimaryDisplayViaSystemTask {
    ...
    $workerPath = "$env:TEMP\airgpu_setprimary_worker.ps1"
    $resultPath = "$env:TEMP\airgpu_setprimary_result.json"
    ...
    Write-Host "  Running display change as SYSTEM in Session 0 (console)..." -ForegroundColor Cyan
    Write-Host ""

    # ── Build the worker script ─────────────────────────────
    # It is self-contained: includes the Win32 type and full enum + set logic.
    $targetName    = $label -replace "'", "''"   # escape for here-string
...
public class DispWorker {
    ...
    public const int  DISP_CHANGE_SUCCESSFUL = 0;
    [DllImport("user32.dll", CharSet=CharSet.Ansi)]
    public static extern bool EnumDisplayDevices(string dev, uint idx, ref DISPLAY_DEVICE dd, uint flags);
    [DllImport("user32.dll", CharSet=CharSet.Ansi)]
    public static extern bool EnumDisplaySettings(string dev, int mode, ref DEVMODE dm);
    [DllImport("user32.dll", CharSet=CharSet.Ansi)]
    public static extern int ChangeDisplaySettingsEx(string dev, ref DEVMODE dm, IntPtr hwnd, uint flags, IntPtr lp);
    [DllImport("user32.dll", CharSet=CharSet.Ansi)]
    public static extern int ChangeDisplaySettingsEx(string dev, IntPtr dm, IntPtr hwnd, uint flags, IntPtr lp);
}
"@ -ErrorAction Stop

$targetName = 'TARGET_NAME_PLACEHOLDER'
$resultFile = 'RESULT_PATH_PLACEHOLDER'

$devices = @()
$idx = 0
while ($true) {
    $dd    = New-Object DispWorker+DISPLAY_DEVICE
    $dd.cb = [System.Runtime.InteropServices.Marshal]::SizeOf($dd)
    if (-not [DispWorker]::EnumDisplayDevices($null, $idx, [ref]$dd, 0)) { break }
    $mirror = [bool]($dd.StateFlags -band [DispWorker]::DISPLAY_DEVICE_MIRRORING_DRIVER)
    $rdp    = $dd.DeviceString -match "RDP|Remote Desktop|IDD"
    if (-not $mirror -and -not $rdp) {
        $dm      = New-Object DispWorker+DEVMODE
        $dm.dmSize = [System.Runtime.InteropServices.Marshal]::SizeOf($dm)
        [DispWorker]::EnumDisplaySettings($dd.DeviceName, -1, [ref]$dm) | Out-Null
        $devices += [PSCustomObject]@{
            Name      = $dd.DeviceName
            Adapter   = $dd.DeviceString
            Active    = [bool]($dd.StateFlags -band [DispWorker]::DISPLAY_DEVICE_ACTIVE)
            Primary   = [bool]($dd.StateFlags -band [DispWorker]::DISPLAY_DEVICE_PRIMARY_DEVICE)
            Width     = $dm.dmPelsWidth; Height = $dm.dmPelsHeight
            PosX      = $dm.dmPositionX; PosY   = $dm.dmPositionY
        }
    }
    $idx++
}

# Find target device by adapter name substring match
$target = $devices | Where-Object { $_.Adapter -like "*$targetName*" } | Select-Object -First 1
if (-not $target) {
    # Fallback: any NVIDIA device
    $target = $devices | Where-Object { $_.Adapter -like "*NVIDIA*" } | Select-Object -First 1
}

$result = @{ Success=$false; Message="Target not found: $targetName"; Devices=@($devices | % { $_.Name + ' | ' + $_.Adapter }) }

if ($target) {
    $offX = $target.PosX; $offY = $target.PosY
    $errors = 0
    foreach ($d in ($devices | Where-Object { $_.Active })) {
        $dm      = New-Object DispWorker+DEVMODE
        $dm.dmSize = [System.Runtime.InteropServices.Marshal]::SizeOf($dm)
        [DispWorker]::EnumDisplaySettings($d.Name, -1, [ref]$dm) | Out-Null
        $dm.dmPositionX = $d.PosX - $offX
        $dm.dmPositionY = $d.PosY - $offY
        $dm.dmFields = 0x00200000
        $flags = [uint32]([DispWorker]::CDS_UPDATEREGISTRY -bor [DispWorker]::CDS_NORESET)
        if ($d.Name -eq $target.Name) { $flags = $flags -bor [uint32][DispWorker]::CDS_SET_PRIMARY }
        $r = [DispWorker]::ChangeDisplaySettingsEx($d.Name, [ref]$dm, [IntPtr]::Zero, $flags, [IntPtr]::Zero)
        if ($r -ne [DispWorker]::DISP_CHANGE_SUCCESSFUL) { $errors++ }
    }
    [DispWorker]::ChangeDisplaySettingsEx($null, [IntPtr]::Zero, [IntPtr]::Zero, 0, [IntPtr]::Zero) | Out-Null
    $result = @{ Success=($errors -eq 0); Message="Applied for '$($target.Name)' / '$($target.Adapter)' errors=$errors"; Devices=@($devices | % { $_.Name + ' | ' + $_.Adapter }) }
}

$result | ConvertTo-Json | Set-Content -Path $resultFile -Encoding UTF8
'@

    # Inject target name and result path
    $workerScript = $workerScript `
        -replace 'TARGET_NAME_PLACEHOLDER', ($label -replace '\\', '\\') `
        -replace 'RESULT_PATH_PLACEHOLDER', ($resultPath -replace '\\', '\\')

    Set-Content -Path $workerPath -Value $workerScript -Encoding UTF8

    # ── Register and run as SYSTEM Scheduled Task ───────────
    try {
        ...
        # SYSTEM + highest privilege = runs in session 0, full console display access
        ...
        Write-Host "  Step 1/3  Scheduled Task registered (SYSTEM / Session 0)..." -ForegroundColor Cyan
        Start-ScheduledTask -TaskName $taskName
        Write-Host "  Step 2/3  Task running -- waiting for result..." -ForegroundColor Cyan
        Write-Log "Scheduled task '$taskName' started for target '$label'."
    } catch {
        Write-Host "  Failed to register/start Scheduled Task: $_" -ForegroundColor Red
        Write-Log "Scheduled task registration failed: $_" -Level "ERROR"
        return $false
    }

    # ── Wait for result file (max 30 s) ─────────────────────
    ...
    if (Test-Path $resultPath) {
        try {
            ...
            $ok  = [bool]$res.Success
            Write-Log "Worker result: $($res.Message)"
            if (-not $ok) {
                Write-Host "  Worker message: $($res.Message)" -ForegroundColor DarkGray
                if ($res.Devices) {
                    Write-Host "  Devices seen by SYSTEM/Session 0:" -ForegroundColor DarkGray
                    $res.Devices | ForEach-Object { Write-Host "    $_" -ForegroundColor DarkGray }
                }
            }
        } catch {
            Write-Log "Could not parse result file: $_" -Level "WARN"
        }
    } else {
        Write-Host "  Timed out waiting for result." -ForegroundColor Red
        Write-Log "Timed out waiting for result file from scheduled task." -Level "ERROR"
    }
