...
function Set-PrimaryDisplayViaSDC {
    ...
    # ── Query with QDC_ALL_PATHS (0x4) so inactive/virtual displays are included ──
    try {
        $numPaths = [uint32]0; $numModes = [uint32]0
        $ret = [DisplayAPI]::GetDisplayConfigBufferSizes(0x4, [ref]$numPaths, [ref]$numModes)
        Write-Host "  [SDC] Buffer sizes: ret=$ret paths=$numPaths modes=$numModes" -ForegroundColor DarkGray
        Write-Log "SDC: BufferSizes ret=$ret paths=$numPaths modes=$numModes"
        if ($ret -ne 0 -or $numPaths -eq 0) {
            Write-Host "  [SDC] GetDisplayConfigBufferSizes failed" -ForegroundColor Red
            return $false
        }
        $paths = New-Object DisplayAPI+DISPLAYCONFIG_PATH_INFO[] $numPaths
        $modes = New-Object DisplayAPI+DISPLAYCONFIG_MODE_INFO[] $numModes
        $ret   = [DisplayAPI]::QueryDisplayConfig(0x4, [ref]$numPaths, $paths, [ref]$numModes, $modes, [IntPtr]::Zero)
        Write-Host "  [SDC] QueryDisplayConfig ret=$ret paths=$numPaths modes=$numModes" -ForegroundColor DarkGray
        Write-Log "SDC: QueryDisplayConfig ret=$ret paths=$numPaths modes=$numModes"
        if ($ret -ne 0) { return $false }
    } catch {
        Write-Host "  [SDC] Query exception: $_" -ForegroundColor Red
        Write-Log "SDC: query exception: $_" -Level "WARN"; return $false
    }

    # ── Log all paths with target names for diagnosis ─────────
    Write-Host "  [SDC] Scanning $numPaths paths..." -ForegroundColor DarkGray
    $targetPathIdx = $null
    for ($p = 0; $p -lt $numPaths; $p++) {
        ...
        $available = $paths[$p].targetInfo.targetAvailable
        $srcLUID   = $paths[$p].sourceInfo.adapterId.LowPart
        $srcModeIdx = $paths[$p].sourceInfo.modeInfoIdx
        Write-Host "  [SDC]   path[$p] fname='$fname' available=$available srcLUID=$srcLUID srcModeIdx=$srcModeIdx" -ForegroundColor DarkGray
        Write-Log "SDC: path[$p] fname='$fname' available=$available srcLUID=$srcLUID srcModeIdx=$srcModeIdx"

        # Match: label contains NVIDIA and adapter LUID matches Target, OR name matches
        $luidMatch = ($Target.AdapterId -ne $null -and
                      $paths[$p].sourceInfo.adapterId.LowPart  -eq $Target.AdapterId.LowPart -and
                      $paths[$p].sourceInfo.adapterId.HighPart -eq $Target.AdapterId.HighPart)
        $nameMatch = ($fname -and $label -like "*$($fname.Split(' ')[0])*")
        $nvidiaMatch = ($label -like "*NVIDIA*" -and $fname -like "*NVIDIA*")

        if ($luidMatch -or $nameMatch -or $nvidiaMatch) {
            $targetPathIdx = $p
            Write-Host "  [SDC]   --> MATCHED path[$p] (luid=$luidMatch name=$nameMatch nvidia=$nvidiaMatch)" -ForegroundColor Cyan
            Write-Log "SDC: matched path[$p] luid=$luidMatch name=$nameMatch nvidia=$nvidiaMatch"
            break
        }
    }

    # ── Log all source modes ──────────────────────────────────
    Write-Host "  [SDC] Source modes:" -ForegroundColor DarkGray
    for ($m = 0; $m -lt $numModes; $m++) {
        $it = $modes[$m].infoType
        $w  = $modes[$m].info.sourceMode.width
        $h  = $modes[$m].info.sourceMode.height
        $x  = $modes[$m].info.sourceMode.position.x
        $y  = $modes[$m].info.sourceMode.position.y
        $adaptLow = $modes[$m].adapterId.LowPart
        Write-Host "  [SDC]   mode[$m] infoType=$it adapterLow=$adaptLow ${w}x${h} at ($x,$y)" -ForegroundColor DarkGray
        Write-Log "SDC: mode[$m] infoType=$it adapterLow=$adaptLow ${w}x${h} at ($x,$y)"
    }

    # ── Find source mode for target path ─────────────────────
    # QDC_ALL_PATHS: inactive paths have modeInfoIdx=0xFFFFFFFF
    # We need to find the mode that belongs to the target adapter
    # by matching mode.adapterId to path.sourceInfo.adapterId
    $targetSrcIdx = $null

    if ($null -ne $targetPathIdx) {
        # Try the modeInfoIdx from the path first
        $si = $paths[$targetPathIdx].sourceInfo.modeInfoIdx
        if ($si -ne 0xFFFFFFFF -and [int]$si -lt $modes.Count -and $modes[[int]$si].infoType -eq 2) {
            $targetSrcIdx = [int]$si
            Write-Log "SDC: target srcIdx from path modeInfoIdx=$targetSrcIdx"
        }

        # If not found (inactive path), search modes by adapter LUID
        if ($null -eq $targetSrcIdx) {
            $tLow  = $paths[$targetPathIdx].sourceInfo.adapterId.LowPart
            $tHigh = $paths[$targetPathIdx].sourceInfo.adapterId.HighPart
            for ($m = 0; $m -lt $modes.Count; $m++) {
                if ($modes[$m].infoType -eq 2 -and
                    $modes[$m].adapterId.LowPart  -eq $tLow -and
                    $modes[$m].adapterId.HighPart -eq $tHigh) {
                    $targetSrcIdx = $m
                    Write-Log "SDC: target srcIdx from mode adapterId match: $m"
                    break
                }
            }
        }
    ...
    # Last resort: pick mode with largest valid resolution
    if ($null -eq $targetSrcIdx) {
        Write-Log "SDC: No path/mode match -- using largest source mode" -Level "WARN"
        $bestArea = 0
        for ($m = 0; $m -lt $modes.Count; $m++) {
            if ($modes[$m].infoType -eq 2) {
                $w = $modes[$m].info.sourceMode.width
                $h = $modes[$m].info.sourceMode.height
                # Skip garbage values (over 10000 pixels wide = corrupt struct read)
                if ($w -gt 0 -and $w -le 10000 -and $h -gt 0 -and $h -le 10000) {
                    $area = [long]$w * [long]$h
    ...
    if ($null -eq $targetSrcIdx) {
        Write-Host "  [SDC] Could not identify target source mode." -ForegroundColor Red
    ...
    $w  = $modes[$targetSrcIdx].info.sourceMode.width
    $h  = $modes[$targetSrcIdx].info.sourceMode.height
    Write-Host "  [SDC] Target srcIdx=$targetSrcIdx ${w}x${h} at ($ox,$oy) -- shifting to (0,0)" -ForegroundColor Cyan
    Write-Log "SDC: shifting srcIdx=$targetSrcIdx by (-$ox,-$oy)"
    ...
    try {
        ...
        Write-Host "  [SDC] SetDisplayConfig ret=$ret (0=success)" -ForegroundColor $(if ($ret -eq 0) { "Green" } else { "Red" })
        Write-Log "SDC: SetDisplayConfig ret=$ret"
        return ($ret -eq 0)
    } catch {
        Write-Host "  [SDC] SetDisplayConfig exception: $_" -ForegroundColor Red
    ...
    # Ensure path is marked active
    $targetPath.flags = 1  # DISPLAYCONFIG_PATH_ACTIVE

    $newPaths = @($targetPath)
    $newModes = @()   # empty -- let Windows assign modes

    # Flags: apply topology change, allow Windows to fill in modes
    $sdcFlags = [DisplayAPI]::SDC_APPLY `
             -bor [DisplayAPI]::SDC_TOPOLOGY_SUPPLIED `
             -bor [DisplayAPI]::SDC_ALLOW_CHANGES `
             -bor [DisplayAPI]::SDC_SAVE_TO_DATABASE

    Write-Host "  [SDC] Applying with path[$targetPathIdx] as sole active display..." -ForegroundColor DarkGray
    Write-Log "SDC: applying targetPathIdx=$targetPathIdx srcAdL=$($targetPath.sourceInfo.adapterId.LowPart)"

    try {
        $ret = [DisplayAPI]::SetDisplayConfig(
            [uint32]$newPaths.Count, $newPaths,
            [uint32]$newModes.Count, $null,
            $sdcFlags)
        Write-Host "  [SDC] SetDisplayConfig ret=$ret (0=success)" -ForegroundColor DarkGray
        Write-Log "SDC: SetDisplayConfig ret=$ret"
        if ($ret -eq 0) { return $true }

        # Retry: supply existing active paths but with target source shifted to (0,0)
        Write-Host "  [SDC] Retry with position shift on active paths..." -ForegroundColor DarkGray
        if ($numPathsAct -gt 0) {
            # Find target in active paths by adapter LUID
            $actTargetSrcIdx = $null
            for ($p = 0; $p -lt $numPathsAct; $p++) {
                if ($actPaths[$p].sourceInfo.adapterId.LowPart -eq $targetPath.sourceInfo.adapterId.LowPart -and
                    $actPaths[$p].sourceInfo.adapterId.HighPart -eq $targetPath.sourceInfo.adapterId.HighPart) {
                    $si = $actPaths[$p].sourceInfo.modeInfoIdx
                    if ($si -ne 0xFFFFFFFF -and [int]$si -lt $actModes.Count) {
                        $actTargetSrcIdx = [int]$si; break
                    }
                }
            }
            if ($null -ne $actTargetSrcIdx) {
                $ox = $actModes[$actTargetSrcIdx].info.sourceMode.position.x
                $oy = $actModes[$actTargetSrcIdx].info.sourceMode.position.y
                Write-Log "SDC: retry shift by (-$ox,-$oy)"
                for ($m = 0; $m -lt $numModesAct; $m++) {
                    if ($actModes[$m].infoType -eq 2) {
                        $actModes[$m].info.sourceMode.position.x -= $ox
                        $actModes[$m].info.sourceMode.position.y -= $oy
                    }
                }
                $sdcFlags2 = [DisplayAPI]::SDC_APPLY `
                          -bor [DisplayAPI]::SDC_USE_SUPPLIED_DISPLAY_CONFIG `
                          -bor [DisplayAPI]::SDC_SAVE_TO_DATABASE `
                          -bor [DisplayAPI]::SDC_ALLOW_CHANGES
                $ret2 = [DisplayAPI]::SetDisplayConfig($numPathsAct, $actPaths, $numModesAct, $actModes, $sdcFlags2)
                Write-Host "  [SDC] Retry ret=$ret2" -ForegroundColor DarkGray
                Write-Log "SDC: retry ret=$ret2"
                return ($ret2 -eq 0)
            }
        }
        return $false
        ...
        Write-Host "  [SDC] SetDisplayConfig exception: $_" -ForegroundColor Red
        Write-Log "SDC: exception: $_" -Level "WARN"
        return $false
    }
}
